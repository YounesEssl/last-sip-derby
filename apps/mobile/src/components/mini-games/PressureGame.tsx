...
export function PressureGame({ active, confirmedScore, onScore }: { active: boolean; confirmedScore?: number | null; onScore: (score: number) => void }) {
  ...
  useEffect(() => {
    ...
    const rounded = Math.min(100, Math.max(0, Math.round(confirmedScore * 10) / 10))
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      if (!lockedRef.current) {
        const next = (((time - startedAtRef.current) % CYCLE_MS) / CYCLE_MS) * 100
  ...
  const stop = () => {
    if (!active || lockedRef.current) return
    lockedRef.current = true
    const rounded = Math.min(100, Math.round(valueRef.current * 10) / 10)
    setValue(rounded)
    setScore(rounded)
    onScore(rounded)
    playGameFeedback(rounded >= 95 ? 'SUCCESS' : 'LOCK')
    if (typeof navigator !== 'undefined' && navigator.vibrate) navigator.vibrate(rounded >= 95 ? [30, 30, 70] : 35)
  }
  ...
  return (
    ...
    >
      <svg viewBox="0 0 100 210" className="h-[min(48vh,370px)] min-h-[260px] w-[42%] max-w-[155px] overflow-visible drop-shadow-[0_0_18px_rgba(0,0,0,.7)]" aria-label={`Jauge à ${formatted} pour cent`}>
        <defs>
          <clipPath id={clipId}>
            <path d="M50 205 L4 5 L96 5 Z" />
          </clipPath>
          <linearGradient id={`${clipId}-fill`} x1="0" y1="1" x2="0" y2="0">
        ...
        <rect x="0" y={210 - displayed * 2.05} width="100" height={displayed * 2.05} fill={`url(#${clipId}-fill)`} clipPath={`url(#${clipId})`} />
        {[20, 40, 60, 80, 100].map((mark) => {
          const y = 210 - mark * 2.05
          const halfWidth = 46 * ((210 - y) / 200)
          return <line key={mark} x1={50 - halfWidth} x2={50 + halfWidth} y1={y} y2={y} stroke="rgba(255,255,255,.28)" strokeWidth="1" />
        })}
